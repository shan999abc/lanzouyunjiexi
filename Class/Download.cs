...
namespace TEST
{
    internal class Download
    {
        ...
        {
            string x = "";
            foreach (Match m in Regex.Matches(page, pattern)) { if (m.Length > 70) { x = m.Value; } }
            return x;
        }
        /// <summary>
        /// 蓝奏云直链解析
    ...
    public class Web : IDisposable
    {
        ...
        internal static async Task<string> UploadData(string Referer, string address, byte[] postdata)
        {
            using (Web Web = new Web(Referer, is_upload: true)) { return Encoding.UTF8.GetString(await Web.Client.UploadDataTaskAsync(address, "POST", postdata)); }
        }
        public void Dispose()
        {
            Dispose(true);
